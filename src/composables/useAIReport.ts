...
## 🔋 核心动力解析 (Fuel)
请用三到五个自然段，每段话是一段连贯的文字（约 100-150 字），深度解析那些被标记为 ⭐Core 和 ✅Yes 的选项。不要只罗列选项名称，不要重复表述用户的选项和态度，而是分析这些需求背后反映了用户怎样的心理机制，以及什么样的人或事物能为其提供长久的滋养。
    ...
    allModules.forEach(m => {
      m.questions.forEach(q => {
        ...
        q.options.forEach((_opt, idx) => {
          ...
          // 2. 数据清洗：如果不是有效数字 (0-5)，强制视为 0 (未表态)
          // 这一步能彻底防止 undefined 出现在文本中
          const safeA = (typeof rawA === 'number' && rawA >= 0 && rawA <= 5) ? rawA : 0;
          const safeB = (typeof rawB === 'number' && rawB >= 0 && rawB <= 5) ? rawB : 0;
          ...
          // A. 需要沟通：一方是“待商议(2)”或“未表态(0)”
          if (a === 2 || b === 2 || (a === 0 && b !== 0) || (a !== 0 && b === 0)) {
            entries.push({ text: line, type: 'discuss' });
          } 
          // B. 致命冲突：一方是“核心需求(4)”，另一方是“禁止(1)”或“不喜欢(5)”
          // (即：我非要 vs 你不行/不喜欢)
          else if ((a === 4 && (b === 1 || b === 5)) || ((a === 1 || a === 5) && b === 4)) {
            entries.push({ text: line, type: 'critical' });
          } 
          // C. 默契共振：双方都在“正向区间(3,4)” 或 双方都在“负向区间(1,5)”
          // (即：我们都喜欢，或者我们都讨厌)
          else if ((a >= 3 && b >= 3) || ((a === 1 || a === 5) && (b === 1 || b === 5))) {
            entries.push({ text: line, type: 'resonance' });
          } 
          // D. 协商让步：剩下的情况 (如 3 vs 1, 3 vs 5, 4 vs 3 等强度不一致但方向未完全对立的情况)
          else {
